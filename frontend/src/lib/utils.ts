...
export const isHappeningNow = (startDate: Date, endDate: Date) => {
    const now = new Date();
    return (
        new Date(startDate).getDate() <= now.getDate() &&
        now.getDate() <= new Date(endDate).getDate()
    );
};
...
export const isRegistrationOpenCheck = (
    registrationOpen: Date,
    registrationClose: Date
) => {
    return (
        new Date(registrationOpen).getTime() < new Date().getTime() &&
        new Date(registrationClose).getTime() > new Date().getTime()
    );
};
...
export const magicButtonProperties = (competition: Competition) => {
    ...
    const registrationOpen = isRegistrationOpenCheck(
    ...
    const magicButtonText = isPast
        ? t("results")
        : isHappeningNow(competition.startDate, competition.endDate)
          ? t("liveResults")
          : t("register");
    const magicButtonLink = isPast
        ? `${WCA_ORIGIN}/competitions/${competition.wcaId}/results/all`
        : isHappeningNow(competition.startDate, competition.endDate)
